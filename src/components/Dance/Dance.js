...
import eventObj from "./events.json";
...
export default function Dance(){
  ...
  const [category,setCategory]=useState(eventObj["NATRAJ"]);
  // useEffect(()=>{
  //   setCategory(eventData[0])
  // }, [])
    ...
    <form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
      ...
      <div class="flex items-center justify-center">
        ...
        <nav>
            <div className="flex items-center justify-center scrollbar-hide px-10 mt-5 mb-5 sm:px-20 text-2xl whitespace-nowrap space-x-10 sm:space-x-20 overflow-x-scroll ">
                ...
                {Object.keys(eventData).map((category)=>
                  <h3 className={DanceCss.btnCss} onClick={handleEventClick}>{category}</h3>
                )}
            </div>
        ...
        return (
          ...
          <img className="w-full" src={event.image_URL} alt="Sunset in the mountains" />
           <div className="mt-1 text-white" style={{backgroundColor:"#F74061"}}>
            ...
            <div className="font-bold text-xl mb-2" style={{color:"#06122E"}}>{(event.maxMembers==1)?"Individual Event":"Team Event"}</div>
    ...
    <div
      ...
      <div className={DanceCss.eventCardExpandedBorder}>
          <div>
            ...
            <div className={DanceCss.eventCardTitle}>{(event.maxMembers==1)?"Individual Event":"Team Event"}</div>
          </div>

          <div className="flex items-center justify-center mt-5 mb-5">
            {event.maxMembers==1?
            <button type="button" class="px-20 py-3 bg-[#06122E] text-[#F74061] font-medium text-xl leading-tight uppercase rounded shadow-md hover:bg-[#06124F] active:shadow-lg transition duration-150 ease-in-out" data-bs-toggle="modal" data-bs-target={"#exampleModalRegister"+teamNumber} onClick={handleClose2}>Register</button>
            :
            <>
              <Button
